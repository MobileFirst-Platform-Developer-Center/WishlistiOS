...
class WishListDataManager: NSObject {
    ...
    func setUpDB(callback:(Bool, [Item]!)->()){
        let manager:IMFDataManager = IMFDataManager.initializeWithUrl(NSUserDefaults.standardUserDefaults().objectForKey("DataProxyCustomServerURL") as! String)
        
        manager.remoteStore(self.remoteStoreName, completionHandler: { (createdStore:CDTStore!, err:NSError!) -> Void in
            if nil != err{
                //error
                print("Error in creating remote store :  \(err.debugDescription)")
                let error:UIAlertView = UIAlertView(title: "Wishlist", message: "There was an error while retrieving the wish list", delegate: nil, cancelButtonTitle: "OK")
                error.show()
            }else{
                self.dataStore = createdStore
                self.dataStore.mapper.setDataType("Item", forClassName: NSStringFromClass(Item.classForCoder()))
                
                print("Successfully created store : \(self.dataStore?.name)")
    ...
    func getWishListItems(callback:(Bool, [Item]!)->()) {
        ...
        if self.dataStore == nil {
            ...
            self.dataStore.performQuery(query, completionHandler: { (results, error) -> Void in
                if nil != error {
                    // Handle error
                    print("could not retrieve all the data from remote store \(error.debugDescription)")
                } else {
    ...
    func saveItemToWishList(item: Item, callback:()->()){
        ...
        self.dataStore?.save(item, completionHandler: { (object, err) -> Void in
            if nil != err{
                print("could not save object to cloudant store \(err.debugDescription)")
                let saveFailure:UIAlertView = UIAlertView(title: "ParkStore", message: "Could not save object to cloudant store", delegate: nil, cancelButtonTitle: "OK")
                saveFailure.show()
            }else{
                print("successfully saved object to cloudant store")
                callback()
            }
        })
    }
