...
//
//  Modifications for this distribution by Cloudant, Inc., Copyright (c) 2014
//  Cloudant, Inc.
...
@property(nonatomic) uint32_t reachabilityFlags;
...
- (id)initWithHostName:(NSString *)hostName {
    ...
    if (self) {
        ...
        if (!_ref ||
            !SCNetworkReachabilitySetCallback(_ref, ClientCallback, &context)) {
            return nil;
        }
    }
    return self;
...
- (BOOL)start {
    ...
    if (!SCNetworkReachabilityScheduleWithRunLoop(_ref, runLoop,
...
- (void)stop {
    if (_runLoop) {
        SCNetworkReachabilityUnscheduleFromRunLoop(_ref, _runLoop,
                                                   kCFRunLoopCommonModes);
        CFRelease(_runLoop);
        _runLoop = NULL;
    }
...
reachabilityKnown = _reachabilityKnown,
...
- (BOOL)reachable {
    // We want 'reachable' to be on, but not any of the flags that indicate that a
    // network interface
